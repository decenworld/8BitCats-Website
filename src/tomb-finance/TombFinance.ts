...
export class TombFinance {
   ...
   */
   ...
   /**
     ...
     * @param address account address
     */
     async mintchibi(account: string, mintAmount: string): Promise<void> {
      let totalCostWei = String(parseFloat(mintAmount) * 800000000000000000);
      try {
      await this.contracts['ChibicatNFT'].mint(account, mintAmount,
              {
        gasLimit: String(parseFloat(mintAmount) * 285000),
        from: account,
        value: totalCostWei,
