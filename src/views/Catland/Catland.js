...
const [mintAmount, setMintAmount] = useState(1);
  ...
  const selectNftStaked = async (index) => {
    ...
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[index].tokenId], 'LandStakingv1'));
  }
  ...
  const stake = async () => {
    await tombFinance.stakeNfts([nftsInWallet[indexOfSelectedNftInWallet].tokenId], 'LandStakingv1');
    reloadNfts();
  }

  const unStake = async () => {
    await tombFinance.unStake(nftsStaked[indexOfSelectedNft].tokenId, 'LandStakingv1');
    reloadNfts();
  }
