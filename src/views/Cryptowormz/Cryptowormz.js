...
const [mintAmount, setMintAmount] = useState(1);
  ...
  const reloadNfts = async () => {
    if (account) {
      let nftsInWalletWithJSON = await tombFinance.getNFTsInWallet(account, 'cryptowormzNFT');
      ...
      let nftsStakedWithJSON = await tombFinance.getNFTsStaked(account, 'cryptowormzNFT', 'cryptowormzStaking');
      ...
      setNftTotalSupply(await tombFinance.nftTotalSupply('cryptowormzNFT'));
      setNftStakedTotalSupply(await tombFinance.nftStakedTotalSupply('cryptowormzNFT', 'cryptowormzStaking'));
    }
  }
  ...
  const getImageFromJSON = async (json) => {
    try {
      const { image, name} = await (await fetch('https://miniversefinance.mypinata.cloud/ipfs/' + json.replace('https://ipfs.io/ipfs/', ''))).json();
      return {
        image: 'https://miniversefinance.mypinata.cloud/ipfs/' + image.replace('https://ipfs.io/ipfs/', ''),
        name,
      };
    } catch(e) {
  ...
  const selectNftStaked = async (index) => {
    ...
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[index].tokenId], 'cryptowormzStaking'));
  }
  ...
  const stake = async () => {
    await tombFinance.stakeNfts([nftsInWallet[indexOfSelectedNftInWallet].tokenId], 'cryptowormzStaking');
    reloadNfts();
  }

  const unStake = async () => {
    await tombFinance.unStake(nftsStaked[indexOfSelectedNft].tokenId, 'cryptowormzStaking');
    reloadNfts();
  }

  const claim = async () => {
    await tombFinance.claim(nftsStaked[indexOfSelectedNft].tokenId, 'cryptowormzStaking');
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[indexOfSelectedNft].tokenId], 'cryptowormzStaking'));
  }

  const approve = async () => {
    await tombFinance.approve('cryptowormzNFT', 'cryptowormzStaking');
  }
    ...
    <Switch>
      <Page>
        ...
        <Route exact path={path}>
          ...
          <h2 style={{ textAlign:'center', marginBottom: '5px'  }}>Cryptowormz</h2>
          <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
            ...
            <span>
             <span style={{fontSize: '20px'}}>Total Minted
               ...
               <br></br>STAKE CRYPTOWORMZ AND EARN CATSHARES
             </span>
                 </Grid>
                     <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
                     <img style={{width: '200px', height:'329', border: '0px black solid'}} src={'https://cryptowormz.fi/config/images/worm2.png'} />
          <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
            ...
            <span style={{ fontSize: '36px' }}>
              { parseInt(nftStakedTotalSupply * 100 / nftTotalSupply) } % Cryptowormz STAKED
            </span>
