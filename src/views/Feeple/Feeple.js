...
const [mintAmount, setMintAmount] = useState(1);
  ...
  const reloadNfts = async () => {
    if (account) {
      let nftsInWalletWithJSON = await tombFinance.getNFTsInWallet(account, 'feepleNFT');
      ...
      let nftsStakedWithJSON = await tombFinance.getNFTsStaked(account, 'feepleNFT', 'feepleStaking');
      ...
      setNftTotalSupply(await tombFinance.nftTotalSupply('feepleNFT'));
      setNftStakedTotalSupply(await tombFinance.nftStakedTotalSupply('feepleNFT', 'feepleStaking'));
    }
  }
  ...
  const selectNftStaked = async (index) => {
    ...
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[index].tokenId], 'feepleStaking'));
  }
  ...
  const stake = async () => {
    await tombFinance.stakeNfts([nftsInWallet[indexOfSelectedNftInWallet].tokenId], 'feepleStaking');
    reloadNfts();
  }

  const unStake = async () => {
    await tombFinance.unStake(nftsStaked[indexOfSelectedNft].tokenId, 'feepleStaking');
    reloadNfts();
  }

  const claim = async () => {
    await tombFinance.claim(nftsStaked[indexOfSelectedNft].tokenId, 'feepleStaking');
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[indexOfSelectedNft].tokenId], 'feepleStaking'));
  }

  const approve = async () => {
    await tombFinance.approve('feepleNFT', 'feepleStaking');
  }
    ...
    <Switch>
      <Page>
        <Route exact path={path}>
          ...
          <h2 style={{ textAlign:'center', marginBottom: '5px'  }}>Feeple</h2>
          <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
            ...
            <span>
             <span style={{fontSize: '20px'}}>Total Minted
               ...
               <br></br>STAKE FEEPLES AND EARN CATSHARES
             </span>
                 </Grid>
                     <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
                     <img style={{width: '200px', height:'329', border: '0px black solid'}} src={require('./example.gif')} />
          <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
            ...
            <span style={{ fontSize: '36px' }}>
              { parseInt(nftStakedTotalSupply * 100 / nftTotalSupply) } % Feeple STAKED
            </span>
