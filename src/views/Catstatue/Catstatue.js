...
const [mintAmount, setMintAmount] = useState(1);
  ...
  const reloadNfts = async () => {
    if (account) {
      let nftsInWalletWithJSON = await tombFinance.getNFTsInWallet(account, 'catstatueNFT');
      ...
      let nftsStakedWithJSON = await tombFinance.getNFTsStaked(account, 'catstatueNFT', 'catstatueStaking');
      ...
      setNftTotalSupply(await tombFinance.nftTotalSupply('catstatueNFT'));
      setNftStakedTotalSupply(await tombFinance.nftStakedTotalSupply('catstatueNFT', 'catstatueStaking'));
    }
  }
  ...
  const selectNftStaked = async (index) => {
    ...
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[index].tokenId], 'catstatueStaking'));
  }
  ...
  const stake = async () => {
    await tombFinance.stakeNfts([nftsInWallet[indexOfSelectedNftInWallet].tokenId], 'catstatueStaking');
    reloadNfts();
  }

  const unStake = async () => {
    await tombFinance.unStake(nftsStaked[indexOfSelectedNft].tokenId, 'catstatueStaking');
    reloadNfts();
  }

  const claim = async () => {
    await tombFinance.claim(nftsStaked[indexOfSelectedNft].tokenId, 'catstatueStaking');
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[indexOfSelectedNft].tokenId], 'catstatueStaking'));
  }

  const approve = async () => {
    await tombFinance.approve('catstatueNFT', 'catstatueStaking');
  }

  const mint = async (amount) => {
    console.log(account);
    await tombFinance.mintStatue(account, amount);
          ...
          <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
            ...
            <span>
             <span style={{fontSize: '20px'}}>Total Minted
               ...
               <br></br>{nftTotalSupply}/999
             </span>
                 </Grid>
                     <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
                     <img style={{width: '140px', height:'210px', border: '0px black solid', imageRendering: "pixelated"}} src={require('./example.png')} />
          <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>

                      <h4 style={{ textAlign:'center', marginBottom: '2px'  }}>1 Cat Statue - 75 FTM</h4>
                  </Grid>
                 </Grid>
              <span>
                      <circleButton
