...
const [mintAmount, setMintAmount] = useState(3);
  ...
  const reloadNfts = async () => {
    if (account) {
      let nftsInWalletWithJSON = await tombFinance.getNFTsInWallet(account, 'trippycatNFT');
      ...
      let nftsStakedWithJSON = await tombFinance.getNFTsStaked(account, 'trippycatNFT', 'trippycatStakingNFT');
      ...
      setNftTotalSupply(await tombFinance.nftTotalSupply('trippycatNFT'));
      setNftStakedTotalSupply(await tombFinance.nftStakedTotalSupply('trippycatNFT', 'trippycatStakingNFT'));
    }
  }
  ...
  const selectNftStaked = async (index) => {
    ...
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[index].tokenId], 'trippycatStakingNFT'));
  }
  ...
  const stake = async () => {
    await tombFinance.stakeNfts([nftsInWallet[indexOfSelectedNftInWallet].tokenId], 'trippycatStakingNFT');
    reloadNfts();
  }
    ...
    let nftsInWalletWithJSON = await tombFinance.getNFTsInWallet(account, 'trippycatNFT');
      for (const nft of nftsInWalletWithJSON){
        await tombFinance.stakeNfts([nft.tokenId], 'trippycatStakingNFT');
      }
  ...
  const unStake = async () => {
    await tombFinance.unStake(nftsStaked[indexOfSelectedNft].tokenId, 'trippycatStakingNFT');
    reloadNfts();
  }

  const claim = async () => {
    await tombFinance.claim(nftsStaked[indexOfSelectedNft].tokenId, 'trippycatStakingNFT');
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[indexOfSelectedNft].tokenId], 'trippycatStakingNFT'));
  }

  const approve = async () => {
    await tombFinance.approve('trippycatNFT', 'trippycatStakingNFT');
  }

  const mint = async (amount) => {
    console.log(account);
    await tombFinance.mintTrippycat(account, amount);
  ...
  <div style={{
    ...
    borderRadius: '15px',
          ...
          <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
                      ...
                      <h4 style={{ textAlign:'center', marginBottom: '2px'  }}>1 Trippy Cat Only 50 FTM</h4>
                  </Grid>
                 </Grid>
              <span>
                      <circleButton
          ...
          <br></br>
            <span style={{ fontSize: '36px' }}>
              { parseInt(nftStakedTotalSupply * 100 / nftTotalSupply) } % Trippy Cats STAKED
            </span>
