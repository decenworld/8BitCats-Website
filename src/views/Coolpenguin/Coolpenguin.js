...
const [mintAmount, setMintAmount] = useState(1);
  ...
  const reloadNfts = async () => {
    if (account) {
      let nftsInWalletWithJSON = await tombFinance.getNFTsInWallet(account, 'coolpenguinNFT');
      ...
      let nftsStakedWithJSON = await tombFinance.getNFTsStaked(account, 'coolpenguinNFT', 'coolpenguinStaking');
      ...
      setNftTotalSupply(await tombFinance.nftTotalSupply('coolpenguinNFT'));
      setNftStakedTotalSupply(await tombFinance.nftStakedTotalSupply('coolpenguinNFT', 'coolpenguinStaking'));
    }
  }
  ...
  const selectNftStaked = async (index) => {
    ...
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[index].tokenId], 'coolpenguinStaking'));
  }
  ...
  const stake = async () => {
    await tombFinance.stakeNfts([nftsInWallet[indexOfSelectedNftInWallet].tokenId], 'coolpenguinStaking');
    reloadNfts();
  }

  const unStake = async () => {
    await tombFinance.unStake(nftsStaked[indexOfSelectedNft].tokenId, 'coolpenguinStaking');
    reloadNfts();
  }

  const claim = async () => {
    await tombFinance.claim(nftsStaked[indexOfSelectedNft].tokenId, 'coolpenguinStaking');
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[indexOfSelectedNft].tokenId], 'coolpenguinStaking'));
  }

  const approve = async () => {
    await tombFinance.approve('coolpenguinNFT', 'coolpenguinStaking');
  }
    ...
    <Switch>
      <Page>
        <Route exact path={path}>
          ...
          <h2 style={{ textAlign:'center', marginBottom: '5px'  }}>Coolpenguin</h2>
          <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
                                      
            <span>
             <span style={{fontSize: '20px'}}>Total Minted
             </span>
               <br></br>{nftTotalSupply}/10000<p></p>
               <br></br>STAKE COOLPENGUIN AND EARN CATSHARES
             </span>
                 </Grid>
                     <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
                     <img style={{width: '200px', height:'329', border: '0px black solid'}} src={'https://ipfs.infura-ipfs.io/ipfs/QmW8xmsfRKaWeNmzqPrKEzgFGVNzN36J3SRx8SFRANAyHE/1611.png'} />
          <Grid container justify="center" spacing={0} style={{marginTop: '10px', marginBottom: '10px'}}>
            ...
            <span style={{ fontSize: '36px' }}>
              { parseInt(nftStakedTotalSupply * 100 / nftTotalSupply) } % Coolpenguin STAKED
            </span>
