...
const [mintAmount, setMintAmount] = useState(3);
  ...
  const reloadNfts = async () => {
    if (account) {
      let nftsInWalletWithJSON = await tombFinance.getNFTsInWallet(account, 'ChibicatNFT');
      ...
      let nftsStakedWithJSON = await tombFinance.getNFTsStaked(account, 'ChibicatNFT', 'Chibicatsstaking');
      ...
      setNftTotalSupply(await tombFinance.nftTotalSupply('ChibicatNFT'));
      setNftStakedTotalSupply(await tombFinance.nftStakedTotalSupply('ChibicatNFT', 'Chibicatsstaking'));
    }
  }
  ...
  const selectNftStaked = async (index) => {
    ...
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[index].tokenId], 'Chibicatsstaking'));
  }
  ...
  const stake = async () => {
    await tombFinance.stakeNfts([nftsInWallet[indexOfSelectedNftInWallet].tokenId], 'Chibicatsstaking');
    reloadNfts();
  }

  const unStake = async () => {
    await tombFinance.unStake(nftsStaked[indexOfSelectedNft].tokenId, 'Chibicatsstaking');
    reloadNfts();
  }

  const claim = async () => {
    await tombFinance.claim(nftsStaked[indexOfSelectedNft].tokenId, 'Chibicatsstaking');
    setReward(await tombFinance.calculateRewards(account, [nftsStaked[indexOfSelectedNft].tokenId], 'Chibicatsstaking'));
  }

  const approve = async () => {
    await tombFinance.approve('ChibicatNFT', 'Chibicatsstaking');
  }

  const mint = async (amount) => {
    console.log(account);
    await tombFinance.mintchibi(account, amount);
